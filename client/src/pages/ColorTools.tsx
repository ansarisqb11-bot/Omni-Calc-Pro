...
const A_TO_Z_PALETTE = Object.entries(COLOR_NAMES)
  ...
  return (
    <div className="space-y-2">
      ...
      >
        ...
        {showPalette ? "Hide" : "Show"} A-Z Color Palette ({A_TO_Z_PALETTE.length} colors)
      </button>
      ...
      {showPalette && (
        <div className="bg-muted/30 rounded-xl p-3 space-y-3 border border-border">
          ...
          <div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-10 gap-1 max-h-48 overflow-y-auto">
            {filteredColors.map(([name, hex]) => (
              <button
                ...
                title={`${name} (${hex})`}
                data-testid={`button-color-${name}`}
              >
                <span className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 bg-black/50 rounded-lg text-[8px] text-white font-medium truncate px-0.5">
                  {name}
                </span>
              </button>
            ))}
          ...
          {filteredColors.length === 0 && (
            <p className="text-center text-sm text-muted-foreground py-4">No colors found</p>
          )}
        </div>
      )}
    </div>
    ...
    <div className="space-y-2">
      ...
      {showPalette && (
        <ColorPalettePicker 
          ...
          selectedHex={parsed ? rgbToHex(parsed) : undefined}
        />
      )}
    </div>
  );
